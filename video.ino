...
void do_keyboard() {
  ...
  if (kb->getNextVirtualKey(&item, 0)) {
    if (item.down) {
      ...
      //
      modifiers =
        item.CTRL << 0 | item.SHIFT << 1 | item.LALT << 2 | item.RALT << 3 | item.CAPSLOCK << 4 | item.NUMLOCK << 5 | item.SCROLLLOCK << 6 | item.GUI << 7;
...
void vdu_sys_video() {
  ...
  switch (mode) {
    ...
    case PACKET_SCRPIXEL:
      ...
      break;
    case PACKET_AUDIO:
      {  // VDU 23, 0, 5, channel, waveform, volume, freq; duration;
